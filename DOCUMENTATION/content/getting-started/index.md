...
1 - Clone cantor on your project root directory:

```bash
git submodule add https://github.com/Laradock/cantor.git
```

Note: If you are not using Git yet for your project, you can use `git clone` instead of `git submodule `.

*To keep track of your Laradock changes, between your projects and also keep Laradock updated [check these docs](/documentation/#track-your-cantor-changes)*
...
*(It's important to rename the cantor folders to unique name in each project, if you want to run cantor per project).*
...
```bash
git clone https://github.com/cantor/cantor.git
```
...
- Upgrade to Docker [Native](https://www.docker.com/products/docker) for Mac/Windows (Recommended). Check out [Upgrading Laradock](/documentation/#upgrading-cantor)
- Use Laradock v3.\*. Visit the [Laradock-ToolBox](https://github.com/cantor/cantor/tree/LaraDock-ToolBox) branch. *(outdated)*
...
1 - Enter the cantor folder and copy `env-example` to `.env`
...
By default the containers that will be created have the current directory name as suffix (e.g. `cantor_workspace_1`). This can cause mixture of data inside the container volumes if you use cantor in multiple projects. In this case, either read the guide for [multiple projects](#B) or change the variable `COMPOSE_PROJECT_NAME` to something unique like your project name.
...
**Note**: All the web server containers `nginx` ..etc depends on `php-fpm`, which means if you run any of them, they will automatically launch the `php-fpm` container for you, so no need to explicitly specify it in the `up` command. If you have to do so, you may need to run them as follows: `docker-compose up -d nginx php-fpm mysql`.


You can select your own combination of containers from [this list](http://cantor.io/introduction/#supported-software-images).
...
**Note:** You can add `--user=cantor` to have files created as your host's user. Example: 

```shell
docker-compose exec --user=cantor workspace bash
```
